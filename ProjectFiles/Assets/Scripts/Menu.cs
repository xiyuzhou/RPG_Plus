...
public class Menu : MonoBehaviourPunCallbacks, ILobbyCallbacks
{
    ...
    private List<GameObject> playerImage = new List<GameObject>();
    public RectTransform playerImageContainer;
    ...
    public override void OnJoinedRoom()
    {
        ...
        photonView.RPC("HardUpdatePlayerImage", RpcTarget.All);
    }
    [PunRPC]
    void UpdateLobbyUI()
    {
    ...
    GameObject CreatePlayerImage()
    {
        GameObject imageObject = Instantiate(playerImagePrefab, playerImageContainer.transform);
        playerImage.Add(imageObject);
        return imageObject;
    }
    [PunRPC]
    void HardUpdatePlayerImage()
    {
        /*foreach (Transform child in playerImageContainer)
        {
            Destroy(child.gameObject);
        }*/
        foreach (GameObject image in playerImage)
        {
            if (image != null)
                Destroy(image);
        }
    ...
    public void onPlayerChangeImage()
    {
        photonView.RPC("UpdatePlayerImage", RpcTarget.All);
    }
    [PunRPC]
    void UpdatePlayerImage()
    {
        for (int i = 0; i < PhotonNetwork.PlayerList.Length; i++)
        {
            var ImageManager = playerImage[i].GetComponent<PlayerImageManager>();
            int[] playerIndex = (int[])PhotonNetwork.PlayerList[i].CustomProperties["plyerImageIndex"];
            ImageManager.imageIndex = playerIndex;
            ImageManager.name = PhotonNetwork.PlayerList[i].NickName;
        }
        for (int i = 0; i < PhotonNetwork.PlayerList.Length; i++)
        {
            var ImageManager = playerImage[i].GetComponent<PlayerImageManager>();
            ImageManager.UpdateImageObject();
        }
    }
